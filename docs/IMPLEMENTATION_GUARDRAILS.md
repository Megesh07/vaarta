...
3. **Never let an LLM set the live risk SCORE.** Tier-0 (deterministic signal engine) is the only engine allowed to set the risk score during an active call — this remains absolute. *(AMENDED by ADR-0002, 2026-07-07:* an LLM MAY now produce live in-call **suggestions**, including generated reply phrasing, but only subject to ADR-0002's mandatory rails — schema-validated + banned-phrase-filtered + injection-defended + fails-closed to the deterministic path + separate opt-in consent. The LLM advises the user what to say; it never touches the score, and the deterministic engine + Manual Mode remain the always-available offline fallback.*)*
